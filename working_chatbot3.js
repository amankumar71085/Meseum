...
function containsTime(str) {
    ...
    // Regular expression for 12-hour format with AM/PM
    const regex12Hour = /^(0?[1-9]|1[0-2]):[0-5]\d\s*[APap][Mm]$/;

    return regex24Hour.test(str) || regex12Hour.test(str);
}
...
function botResponse(userInput) {
    ...
    if (step === 1) {
        ...
        if (containsTime(userInput)) {
            ...
            response = `Please enter time in a valid format. (eg. 10:10 AM or 23:23)`
        }
    } else if (step === 4) {
        if (userInput === "yes") {
